...
// ─── Store switcher ──────────────────────────────────────────────────────────
(function() {
  const modal     = document.getElementById('store-switcher-modal');
  const body      = document.getElementById('store-switcher-body');
  const openBtn   = document.getElementById('switch-store-btn');
  const closeBtn  = document.getElementById('store-switcher-close');
  ...
  function showPasswordForm(errorMsg) {
    body.innerHTML = `
      <div style="margin-bottom:12px;font-size:13px;color:#aaa;">Enter super password to list stores.</div>
      <input type="password" id="sw-pw" placeholder="Super password"
        style="width:100%;background:#1a1a2e;color:#fff;border:1px solid #3a3a55;border-radius:6px;padding:8px 10px;font-size:14px;outline:none;margin-bottom:8px;">
      ${errorMsg ? `<div style="color:#e74c3c;font-size:12px;margin-bottom:8px;">${errorMsg}</div>` : ''}
      <button id="sw-login-btn"
        style="width:100%;background:#C0392B;color:#fff;border:none;border-radius:6px;padding:9px;font-size:14px;font-weight:600;cursor:pointer;">
        Continue
      </button>
    `;
    const pwInput = document.getElementById('sw-pw');
    const loginBtn = document.getElementById('sw-login-btn');
    ...
    async function doLogin() {
      ...
      try {
        const res = await fetch(`${API_BASE}/api/super-login`, {
          method: 'POST',
        ...
        if (res.ok) {
          ...
          showTenantList(data.tenants || []);
        } else {
          showPasswordForm('Incorrect password. Try again.');
        }
      } catch {
        showPasswordForm('Cannot reach server. Check connection.');
      }
    }
  ...
  function showTenantList(tenants) {
    if (!tenants.length) {
      body.innerHTML = '<p style="text-align:center;color:#aaa;font-size:14px;padding:16px 0;">No active stores found.</p>';
      return;
    }
    body.innerHTML = tenants.map(t => `
      <button data-slug="${t.slug}"
        style="display:flex;align-items:center;gap:10px;width:100%;background:#1a1a2e;border:1px solid #3a3a55;border-radius:8px;padding:10px 12px;color:#fff;font-size:14px;cursor:pointer;margin-bottom:8px;text-align:left;">
        <span style="width:32px;height:32px;border-radius:50%;background:#C0392B;display:flex;align-items:center;justify-content:center;font-weight:700;flex-shrink:0;">
          ${(t.name || t.slug).charAt(0).toUpperCase()}
        </span>
        <span style="font-weight:600;">${t.name || t.slug}</span>
      </button>
    `).join('');
    ...
    body.querySelectorAll('button[data-slug]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try {
          const res = await fetch(`${API_BASE}/api/tenants/select`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          if (res.ok) {
            ...
            if (typeof setTenantSession === 'function') setTenantSession(tenant);
            closeModal();
            fetchShopName();
            const { from, to } = getDateRange(currentPreset);
            loadData(from, to);
